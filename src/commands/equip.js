...
function formatEquipmentItem(label, item) {
    ...
    const stats = [];
    if (item.atk) stats.push(`ATK+const { getPlayer, savePlayer, recalculateStats } = require('../core/player/playerService');
const { inventoryCategoryMenu } = require('../menus/inventoryMenu');
const { Markup } = require('telegraf');
...
function unequipSlot(player, slot) {
    ...
    if (!item) return { ok: false, message: `❌ Nada equipado neste slot (${slot}).` };
...
async function handleUnequipItem(ctx) {
    try {
        ...
        // Recarregar a tela de inventário da categoria atual (opcional: voltar para a categoria anterior)
        // Vamos voltar ao menu principal de inventário para evitar inconsistências
        await ctx.editMessageText('🎒 *INVENTÁRIO*', { parse_mode: 'Markdown', ...inventoryCategoryMenu() });
    } catch (error) {
        console.error('Erro ao desequipar:', error);
        await ctx.answerCbQuery('❌ Erro interno ao desequipar.', true);
    }
}
