...
function applyTurnEffects(fight) {
  ...
  if (fight.enemy.bleedTurns > 0) {
    const bleedDamage = Math.max(
      1,
      Math.floor(fight.enemy.maxHp * 0.05)
    );

    fight.enemy.hp = Math.max(
      0,
      fight.enemy.hp - bleedDamage
    );

    fight.logs.push(
      `🩸 Sangramento causou *${bleedDamage}* dano.`
    );
  ...
  if (fight.enemy.poisonTurns > 0) {
    const poisonDamage = Math.max(
      1,
      Math.floor(fight.enemy.maxHp * 0.04)
    );

    fight.enemy.hp = Math.max(
      0,
      fight.enemy.hp - poisonDamage
    );

    fight.logs.push(
      `☠️ Veneno causou *${poisonDamage}* dano.`
    );
...
function useSoul(fight, soulIndex) {
  ...
  if (!soul) {
    fight.logs.push(
      '❌ Nenhuma alma equipada neste slot.'
    );
...
function processEnemyTurn(fight) {
  ...
  if (fight.enemy.frozen) {
    fight.logs.push(
      `❄️ ${fight.enemy.name} está congelado e perdeu o turno.`
    );
  ...
  if (fight.player.hp <= 0) {
    fight.status = 'loss';
    fight.logs.push(
      `☠️ Você foi derrotado por ${fight.enemy.name}...`
    );
  }
