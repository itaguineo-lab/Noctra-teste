...
function getBalance(player, currency) { return Number(player[currency] || 0); }
function canPay(player, currency, price) { return getBalance(player, currency) >= price; }
...
function addEquipment(player, item) {
    if (player.inventory.length >= player.maxInventory) return { success: false, message: '❌ Inventário cheio.' };
...
function addCosmetic(player, item) {
    const alreadyOwned = player.cosmetics.some(cosmetic => cosmetic.id === item.id);
    if (alreadyOwned) return { success: false, message: '❌ Você já possui este cosmético.' };
    player.cosmetics.push({ id: item.id, name: item.name });
    return { success: true, message: `✨ Cosmético ${item.name} desbloqueado!` };
}
...
function addEnergyRefill(player, item) {
    ...
    return { success: true, message: `⚡ Você recuperou ${amount} de energia!` };
}

function addKey(player, item) {
    const amount = item.value || 1;
    player.keys = (player.keys || 0) + amount;
    return { success: true, message: `🗝️ Você recebeu ${amount} chave(s) de masmorra!` };
}
...
function processPurchase(player, item) {
    ...
    if (!paid) return { success: false, message: '❌ Saldo insuficiente.' };
