...
async function handleBuy(ctx) {
    try {
        ...
        if (!item) return ctx.answerCbQuery('Item inválido.', { show_alert: true });
        ...
        // Redireciona para a loja correta
        if (item.shop === 'village') return handleShopVillage(ctx);
        if (item.shop === 'castle') return handleShopCastle(ctx);
        if (item.shop === 'arena') return handleShopArena(ctx);
