...
async function handleShopVillage(ctx) {
    return renderTab(ctx, 'village', '🏘️ *Loja da Vila*');
}

async function handleShopCastle(ctx) {
    return renderTab(ctx, 'castle', '🏰 *Loja do Castelo*');
}

async function handleShopArena(ctx) {
    return renderTab(ctx, 'arena', '⚔️ *Loja da Arena*');
}
...
async function handleBuy(ctx) {
    try {
        ...
        await ctx.answerCbQuery(
            result.message || '✅ Compra realizada!',
            { show_alert: true }
        );
