...
async function editMessage(ctx, text, options = {}) {
    ...
    } catch { await ctx.reply(text, options); }
}
...
async function finishFight(ctx, fight, turnCount, damageDealt, damageReceived) {
    ...
    if (fight.status === 'win') {
        ...
        if (rewards.loot.length) msg += `║ 🎁 ${rewards.loot.join('\n║ 🎁 ')}\n`;
        if (rewards.leveledUp) {
            msg += `╠════════════════════════╣\n║ 🌟 *LEVEL UP!* Nv ${oldLevel} → ${player.level}\n║ ❤️ HP: ${player.maxHp} (+${player.maxHp - (oldLevel ? player.maxHp - 20 : 0)})\n║ ⚔️ ATK: ${player.atk}  🛡️ DEF: ${player.def}\n`;
        } else {
...
async function handleAttack(ctx) {
    ...
    if (fight.player.buffs && fight.player.buffs.length) {
        fight.player.buffs = fight.player.buffs.filter(buff => { buff.remainingTurns--; return buff.remainingTurns > 0; });
        const player = getPlayer(ctx.from.id);
    ...
    player.energy = fight.player.energy;
    savePlayer(ctx.from.id, player);
    if (fight.status !== 'ongoing') return finishFight(ctx, fight, fight.turnCount, fight.totalDamageDealt, fight.totalDamageReceived);
    return editMessage(ctx, renderFightText(fight, player), { parse_mode: 'Markdown', ...combatMenu() });
}
