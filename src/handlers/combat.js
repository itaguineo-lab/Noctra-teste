...
function syncFightBuffsToPlayer(playerId, fight) {
    ...
    player.buffs = Array.isArray(fight.player.buffs)
        ? fight.player.buffs.map(buff => ({ ...buff }))
        : [];
...
function tickFightBuffs(playerId, fight) {
    ...
    fight.player.buffs = fight.player.buffs
        .map(buff => ({
            ...buff,
            remainingTurns: Math.max(0, (Number(buff.remainingTurns) || 0) - 1)
        }))
...
async function handleHunt(ctx) {
    ...
    if (!enemy) {
        return ctx.reply('❌ Nenhum inimigo disponível neste mapa.');
    }
    ...
    fight.player.buffs = Array.isArray(player.buffs)
        ? player.buffs.map(buff => ({ ...buff }))
        : [];
...
async function useConsumable(ctx, type) {
    ...
    playerUpdated.buffs = Array.isArray(fight.player.buffs)
        ? fight.player.buffs.map(buff => ({ ...buff }))
        : [];
