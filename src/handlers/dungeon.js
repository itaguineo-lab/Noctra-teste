...
async function renderDungeonBattle(ctx, player) {
    ...
    if (!enemy) {
        return ctx.reply('❌ Erro: inimigo não encontrado.');
    }
...
async function handleDungeonAttack(ctx) {
    ...
    const isCrit = Math.random() * 100 < (player.crit || 5);
    let playerDamage = Math.max(1, player.atk - Math.floor(enemy.atk / 4));
    ...
    if (enemy.hp > 0) {
        const enemyDamage = Math.max(1, enemy.atk - Math.floor(player.def / 2));
        ...
        return ctx.reply(
            ...
            {
                parse_mode: 'Markdown',
                ...Markup.inlineKeyboard([ [ Markup.button.callback('⚔️ Continuar', 'dungeon_attack') ] ])
            }
        );
    }
...
async function handleEnemyDefeat(ctx, player, enemy) {
    ...
    return ctx.reply(
        ...
        {
            parse_mode: 'Markdown',
            ...Markup.inlineKeyboard([ [ Markup.button.callback('➡️ Próxima Sala', 'dungeon_next_room') ] ])
        }
    );
}
